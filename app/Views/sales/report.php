...
<div class="card">
    ...
    <div class="card-body">
        <div id="example1_wrapper" class="dataTables_wrapper dt-bootstrap4">
            <div class="row">
                <form action="<?= site_url('sale/report') ?>" method="get">
            ...
            <div class="row">
                <div class="col-sm-12" id="print-area">
                    <table class="table table-sm table-striped datatables-report-sale">
                        <thead>
                            <tr>
                                <th>No</th>
                                <th>No Faktur</th>
                        ...
                        <tbody>
                            ...
                            <?php foreach ($sales as $row) : ?>
                                <tr>
                                    <td><?= $no++; ?></td>
                                    <td><?= $row['detjual_faktur']; ?></td>
                                    <td><?= $row['jual_tgl']; ?></td>
                                    <td><?= $row['namaproduk']; ?></td>
                                    <td><?= $row['detjual_hargajual']; ?></td>
                                    <td><?= $row['detjual_jml']; ?></td>
                                    <td><?= $row['detjual_subtotal']; ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                        <tr>
                            <td colspan="6"><b>Total</b></td>
                            <td><?= $salesTotal['total_jumlah']; ?></td>
                        </tr>
                    </table>
                </div>
            </div>
...
<script>
    ...
    function deleteFilter() {
        $.ajax({
            type: "POST",
            url: "<?= site_url('sale/deleteFilter'); ?>",
            data: {
            ...
            success: function(response) {
                if (response.success) {
                    alert('Tes Cetak Struk');
                    window.location.reload();
                }
            },
            error: function(xhr, thrownError) {
                // alert(`${xhr.status} ${xhr.responseText} ${thrownError}`);
                alert('Perintah tidak diizinkan! Isi Periode terlebih dahulu.');
            }
        });
    }

    $(document).ready(function() {
        $('.datatables-report-sale').DataTable();
    });
</script>
